...
店铺签到，店铺Token默认从本地环境变量DPQDTOKEN中获取，若本地无则从远端获取。

Fix by HarbourJ
TG: https://t.me/HarbourToulu

环境变量:
DPQDTOKEN: token1&token2
仓库不再提供token
*/

let token = []
if (process.env.DPQDTOKEN) {
  if (process.env.DPQDTOKEN.includes('\n')) {
    token = [...process.env.DPQDTOKEN.split('\n'),...token]
  } else {
    token = [...process.env.DPQDTOKEN.split('&'),...token]
  }
}
