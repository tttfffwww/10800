...
//看视频
function accountInfo(timeout = 0) {
    ...
    return new Promise((resolve) => {
        ...
        let url = {
            //url: `https://dj.palmestore.com/zyuc/api/user/accountInfo${dreferfer}&timestamp=${time1}`,
        ...
        $.get(url, async (err, resp, data) => {
            try {
				...
				//console.log(result);
                if (result.code == 0) {
                    console.log(`【用户】：${result.body.userInfo.userNick}\n`)
					console.log(`【登录状态】：${result.body.isLogin}\n`)

                    console.log(`【今日阅读】：${result.body.read.time}\n`)
					console.log(`【金币数】：${result.body.gold.goldAmount}\n`)
...
function djxsvideo(timeout = 0) {
    ...
    return new Promise((resolve) => {
		...
		//console.log(`https://dj.palmestore.com/zycl/gold/receive${dreferfer}&type=videoTask&videoId=158`);
        $.get(url, async (err, resp, data) => {
            try {
...
function djxsvideo(timeout = 0) {
    ...
    return new Promise((resolve) => {
        ...
        dreferfer=djxsReferer.match(/index?(\S.+)/)[1]
        let url = {
            url: `https://dj.palmestore.com/zycl/gold/index/refresh${dreferfer}&type=videoTask&videoId=158`,
            headers: hd,
        }
		//console.log(`https://dj.palmestore.com/zycl/gold/index/refresh${dreferfer}&type=videoTask&videoId=158`);
        $.get(url, async (err, resp, data) => {
            try {
